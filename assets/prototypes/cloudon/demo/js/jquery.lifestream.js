        ...
        for ( ; i < length; i++ ) {
          ...
          if ( item.html ) {
            $('<li class="'+ settings.classname + '-' +
               item.config.service + '">').data( "ownername", item.config.user )
...
$.fn.lifestream.feeds.bitly = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      created:'${ownername} created URL <a href="${short_url}" title="${title}">'
        + '${short_url}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.citeulike = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      saved: '${ownername} saved <a href="${href}">${title}</a> by ${authors}'
    },
    config.template),
...
$.fn.lifestream.feeds.dailymotion = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      uploaded: '${ownername} uploaded a video <a href="${link}">${title[0]}</a>'
    },
    config.template),
...
$.fn.lifestream.feeds.delicious = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      bookmarked: '${ownername} bookmarked <a href="${u}">${d}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.deviantart = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '${ownername} posted <a href="${link}">${title}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.disqus = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      post: '${ownername} commented on <a href="${url}">${thread.title}</a>',      
      thread_like: '${ownername} liked <a href="${url}">${thread.title}</a>'
    },
    config.template),
...
$.fn.lifestream.feeds.dribbble = function( config, callback ) {
    ...
    var template = $.extend({},
      {
        posted: '${ownername} posted a shot <a href="${url}">${title}</a>'
      },
      config.template);
...
$.fn.lifestream.feeds.facebook_page = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      wall_post: '${ownername} post on wall <a href="${link}">${title}</a>'
    },
    config.template),
...
$.fn.lifestream.feeds.flickr = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '${ownername} posted a photo <a href="${link}">${title}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.foomark = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      bookmarked: '${ownername} bookmarked <a href="${url}">${url}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.formspring = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      answered: '${ownername} answered a question <a href="${link}">${title}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.forrst = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '${ownername} posted a ${post_type} '
...
$.fn.lifestream.feeds.gimmebar = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      bookmarked: '${ownername} bookmarked <a href="${short_url}">${title}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.github = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      commitCommentEvent: '${ownername} commented on <a href="http://github.com/'
      + '${status.repo.name}">${status.repo.name}</a>',
      createBranchEvent: '${ownername} created branch <a href="http://github.com/'
      + '${status.repo.name}/tree/${status.payload.ref}">'
      + '${status.payload.ref}</a> at <a href="http://github.com/'
      + '${status.repo.name}">${status.repo.name}</a>',
      createRepositoryEvent: '${ownername} created repository <a href="http://github.com/'
      + '${status.repo.name}">${status.repo.name}</a>',
      createTagEvent: '${ownername} created tag <a href="http://github.com/'
      + '${status.repo.name}/tree/${status.payload.ref}">'
      + '${status.payload.ref}</a> at <a href="http://github.com/'
      + '${status.repo.name}">${status.repo.name}</a>',
      deleteBranchEvent: '${ownername} deleted branch ${status.payload.ref} at '
      + '<a href="http://github.com/${status.repo.name}">'
      + '${status.repo.name}</a>',
      deleteTagEvent: '${ownername} deleted tag ${status.payload.ref} at '
      + '<a href="http://github.com/${status.repo.name}">'
      + '${status.repo.name}</a>',
      followEvent: '${ownername} started following <a href="http://github.com/'
      + '${status.payload.target.login}">${status.payload.target.login}</a>',
      forkEvent: 'forked <a href="http://github.com/${status.repo.name}">'
      + '${status.repo.name}</a>',
      gistEvent: '${ownername} ${status.payload.action} gist '
      + '<a href="http://gist.github.com/${status.payload.gist.id}">'
      + '${status.payload.gist.id}</a>',
      issueCommentEvent: '${ownername} commented on issue <a href="http://github.com/'
      + '${status.repo.name}/issues/${status.payload.issue.number}">'
      + '${status.payload.issue.number}</a> on <a href="http://github.com/'
      + '${status.repo.name}">${status.repo.name}</a>',
      issuesEvent: '${ownername} ${status.payload.action} issue '
      ...
      + '${status.repo.name}</a>',
      pullRequestEvent: '${ownername} ${status.payload.action} pull request '
      + '<a href="http://github.com/${status.repo.name}/pull/'
      + '${status.payload.number}">${status.payload.number}</a> on '
      + '<a href="http://github.com/${status.repo.name}">'
      + '${status.repo.name}</a>',
      pushEvent: '${ownername} pushed to <a href="http://github.com/${status.repo.name}'
      + '/tree/${status.payload.ref}">${status.payload.ref}</a> at '
      + '<a href="http://github.com/${status.repo.name}">'
      + '${status.repo.name}</a>',
      watchEvent: '${ownername} started watching <a href="http://github.com/'
      + '${status.repo.name}">${status.repo.name}</a>'
    },
    config.template),
